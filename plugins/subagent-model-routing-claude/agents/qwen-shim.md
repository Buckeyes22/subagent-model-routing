...
name: qwen-shim
description: >-
  Transports a single shell command invoking ~/.claude/scripts/qwen-shim.sh and
  returns stdout verbatim. Dispatches a one-shot prompt through the installed
  Qwen Code CLI (local/self-hosted OpenAI-compatible endpoints). Do not use it
  for OpenCode provider routes.
model: sonnet
color: purple
---
