...
Do not use Claude-only concepts here. In Codex, there is no Claude-only DAG runtime, no Claude transport-agent prose, and no Claude command wrapper. The Codex path is direct shell orchestration plus Codex-native subagents only when the current Codex surface exposes them and the user explicitly asks for parallel agents.
...
For dependency work in Codex:

1. Keep the graph in the main Codex thread.
